...
public partial class FootballSchoolContext : DbContext
{
    ...
    protected override void OnConfiguring(DbContextOptionsBuilder optionsBuilder)
    {
        ...
        {
            optionsBuilder.UseSqlServer("Server=.\\SQLEXPRESS;Database=FootballSchool;Trusted_Connection=True;TrustServerCertificate=True;");
        }
    }
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<User>(entity =>
        {
            ...
            entity.Property(e => e.Role).HasDefaultValue("Parent");
        });

        modelBuilder.Entity<Attendance>(entity =>
        {
            entity.HasKey(e => e.AttendanceId).HasName("PK__Attendan__57FA4934437C3D8E");
            entity.HasOne(d => d.Student).WithMany(p => p.Attendances).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_Attendance_Student");
            entity.HasOne(d => d.Training).WithMany(p => p.Attendances).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_Attendance_Training");
        });
        ...
        modelBuilder.Entity<Training>(entity =>
        {
            ...
            entity.HasOne(d => d.Coach).WithMany(p => p.Training).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_Training_Coach");
            entity.HasOne(d => d.Facility).WithMany(p => p.Training).HasConstraintName("FK_Training_Facility");
            entity.HasOne(d => d.Team).WithMany(p => p.Training).OnDelete(DeleteBehavior.ClientSetNull).HasConstraintName("FK_Training_Team");
        });
